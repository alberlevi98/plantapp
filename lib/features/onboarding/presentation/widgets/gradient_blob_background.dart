...
/// units of the app's reference frame, then scaled by the Responsive scope.
class GradientBlobBackground extends StatelessWidget {
  ...
  // constructor.
  //
  // Every number below is a *named field* of a design record (left, top,
  // width, height, opacity) transcribed from Figma, which is why it lives
  // here as data rather than in a dimensions file: naming each one
  // individually would produce forty constants used exactly once.
    ...
    return DecoratedBox(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
          begin: OnboardingBackgroundDimensions.gradientBegin,
          end: OnboardingBackgroundDimensions.gradientEnd,
          colors: <Color>[
            context.appColors.onboardingGradientStart,
            context.appColors.onboardingGradientEnd,
          ],
          stops: OnboardingBackgroundDimensions.gradientStops,
        ),
      ),
      child: Stack(
        fit: StackFit.expand,
          ...
          RepaintBoundary(
            child: ImageFiltered(
              imageFilter: ui.ImageFilter.blur(
                sigmaX: AppBlur.backgroundBlob,
                sigmaY: AppBlur.backgroundBlob,
              ),
              child: Stack(
                clipBehavior: Clip.none,
                children: <Widget>[
                  for (final _Blob blob in _blobs) _BlobView(blob: blob),
                ],
              ),
            ),
          ),
...
class _BlobView extends StatelessWidget {
  const _BlobView({required this.blob});

  final _Blob blob;

  @override
  Widget build(BuildContext context) {
    // Both axes scale off the *width* factor, not width/height
    // independently — otherwise a device whose aspect ratio differs from the
    // design frame would stretch every blob (including the five that are
    // meant to stay perfectly round) into an unintended oval.
    final double width = context.w(blob.width);
    final double height = context.w(blob.height);
    ...
    return Positioned(
      ...
      child: Opacity(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            color: blob.color,
            // Radius.elliptical(w/2, h/2) draws a perfect circle when
            // width == height (the five round blobs) and a true oval for
            // Vector 160, so one shape path covers both cases.
            borderRadius: BorderRadius.all(
              Radius.elliptical(
                width / OnboardingBackgroundDimensions.ellipseDivisor,
                height / OnboardingBackgroundDimensions.ellipseDivisor,
              ),
            ),
          ),
        ),
...
class _Blob {
  ...
  /// All values are in design units of the [AppDimensions.designWidth] frame.
  final double left;
  final double top;
  final double width;
  final double height;
